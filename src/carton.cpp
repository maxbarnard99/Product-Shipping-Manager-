...
Carton::Carton(Address address, double weight, double side1, 
                double side2, double side3) : ShippingItem(address, weight, side1) {
      // try{
        set_measurements(side1, side2, side3);
      // }
...
/**
 * @brief - computes and returns lenght plus girth
 *
...
/**
 * @brief displays a carton
 * 
 */
void Carton::Display(std::ostream &out) const {
    out << fixed << setprecision(1) << "Carton: " << weight_ << " lbs. " << length_ << " x "
        << width_ << " x " << height_;
}
